...
class Test(unittest.TestCase):
    ...
    def test_full_flow(self):
        ...
        for i, response_record in enumerate(response["records"]):
            ...
            self.assertEqual(response_record["recordId"], input_record["recordId"])
            self.assertEqual(response_record["data"], input_record["data"])
            self.assertEqual(response_record["result"], "Ok")

        # CHECK HTTP CALL
        self.assertEqual(mock_http_client.call_count, 1)
        ...
        self.assertEqual(len(sent_logs), 4)
